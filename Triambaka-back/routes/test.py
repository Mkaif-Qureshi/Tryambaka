...
#             delta = float(delta)
#         except ValueError:
#             return jsonify({"error": "'key' must be an integer and 'delta' must be a float"}), 400
...
#         # Calculate BER for the watermarked image
#         ber = extract_watermark(output_path, key, delta)
...
#         # Add BER to response headers
#         response.headers['X-BER'] = str(ber)
