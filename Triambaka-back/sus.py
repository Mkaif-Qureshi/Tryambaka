...
# --- Helper functions for watermark scrambling ---
...
def distinguish_watermarked_image(image_path, key, delta=None, threshold=0.2):
    expected_wm, extracted_wm, ber = extract_watermark(image_path, key, delta)
    
    if ber < threshold:
        print("The image is watermarked.")
        print(f"Bit Error Rate (BER): {ber:.4f}")
    else:
        print("The image is not watermarked.")
        print(f"Bit Error Rate (BER): {ber:.4f}")
    
    plt.figure(figsize=(10, 5))
    plt.subplot(1, 2, 1)
    plt.imshow(expected_wm, cmap='gray')
    plt.title("Expected (Scrambled) Watermark")
    
    plt.subplot(1, 2, 2)
    plt.imshow(extracted_wm, cmap='gray')
    plt.title("Extracted (Scrambled) Watermark")
...
if __name__ == "__main__":
    ...
    print("\nChecking watermarked image:")
    distinguish_watermarked_image(watermarked_image, key, delta=delta_value)
